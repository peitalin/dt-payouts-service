...
impl ToSql<Text, Pg> for Currency {
    fn to_sql<W: std::io::Write>(
        ...
        let currency = self.as_string();
        ToSql::<Text, Pg>::to_sql(&currency, out)
    }
}
impl FromSql<Text, Pg> for Currency {
    fn from_sql(maybe_bytes: Option<&[u8]>) -> diesel::deserialize::Result<Self> {
        let currency = <String as FromSql<Text, Pg>>::from_sql(maybe_bytes)
            .expect("Error parsing Currency: <String as FromSql<Text, Pg>>");
        Ok(Currency::from_str(&currency)?)
    }
}
...
impl std::fmt::Display for ParseCurrencyError {
    fn fmt(&self, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        fmt.write_str(&format!("ParseCurrency error: {:?}", self))
    }
}
...
#[test]
fn currency_to_str_is_lowercase() {
    let currency = format!("{}", Currency::USD);
    assert_eq!(currency, String::from("usd"))
}

#[test]
fn deserializes_currency_usd() {
    let currency = Currency::from_str("usd").ok();
